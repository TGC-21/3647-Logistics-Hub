...
describe('buildContextWindow', () => {
  it('keeps the latest request and complete recent tool-call/result pairs', () => {
    ...
    expect(result.messages[2].tool_calls[0].id).toBe('new')
    expect(result.messages[3].tool_call_id).toBe('new')
  })
  ...
  it('includes inventory guidance in the system prompt', () => {
    const result = buildContextWindow([{ role: 'user', content: 'Check inventory' }])
    expect(result.messages[0].content).toContain('Inventory/component domain notes')
    expect(result.messages[0].content).toContain('InventoryInstanceService.createInstance')
  })
})
