// api/change-log.js — Vercel serverless function
//
// Thin, action-dispatched route for ChangeLogService — first read-side
// surface change-log has ever had server-side (see ChangeLogService.js's
// own doc comment for why that gap mattered: no non-browser caller,
// including the eventual agent harness, could ask "what happened to
// this entity" before this route existed).
//
// AUTH DECISION — same as every other route in this migration pass (see
// api/categories.js for the full reasoning): no gate. change_log's RLS
// is already `using (true)` and src/changeLog.js's reads have always
// run straight from the browser with the anon key, so this route isn't
// narrowing anything — it's just giving the same read a home reachable
// by callers that aren't the browser too.
//
// POST /api/change-log
//   { action: 'entityHistory',   entityType, entityId }
//   { action: 'commit',          commitId }
//   { action: 'cascadeChildren', causedByEntityType, causedByEntityId }
//   { action: 'recentActivity',  limit? }

import { applyCors } from './_lib/onshape.js'
import { ChangeLogService } from '../src/services/ChangeLogService.js'
import { statusForError } from '../src/repositories/errors.js'

export default async function handler(req, res) {
  if (req.method === 'OPTIONS') { applyCors(res); return res.status(204).end() }
  applyCors(res)
  if (req.method !== 'POST') return res.status(405).json({ error: 'POST only' })

  const body = req.body ?? {}
  const service = new ChangeLogService()

  try {
    switch (body.action) {
      case 'entityHistory': {
        const rows = await service.fetchEntityHistory(body.entityType, body.entityId)
        return res.status(200).json({ success: true, rows })
      }

      case 'commit': {
        const rows = await service.fetchCommit(body.commitId)
        return res.status(200).json({ success: true, rows })
      }

      case 'cascadeChildren': {
        const rows = await service.fetchCascadeChildren(body.causedByEntityType, body.causedByEntityId)
        return res.status(200).json({ success: true, rows })
      }

      case 'recentActivity': {
        const rows = await service.fetchRecentActivity(body.limit || 50)
        return res.status(200).json({ success: true, rows })
      }

      default:
        return res.status(400).json({
          error: `Unknown action "${body.action}" — expected one of: entityHistory, commit, cascadeChildren, recentActivity.`,
        })
    }
  } catch (err) {
    console.error('[change-log]', err)
    return res.status(statusForError(err)).json({ error: err.message ?? 'Internal server error' })
  }
}