...
7. **Material field values** — fixed enum, not free text (mirrors `Spacer Type`'s enum pattern). Candidate option list: `Aluminum`, `Polycarbonate`, `Acrylic`, `Steel`, `Other` — final list TBD at implementation time, easy to extend since it's just `requiredKeysConfig` options like any other category.
8. **Multi-offset / stepped plates** — tagged `needs_review` with an explicit reason distinguishing them from a generic non-match (e.g. `warnings: ['Found N distinct thickness offsets — likely a stepped or ribbed plate, not classified automatically.']`), so the review queue is actually useful for triage rather than lumping stepped plates in with unrelated detection failures.

## Sheet-Metal Exclusion (resolved: needs a real code-level check)

Since part naming won't reliably flag sheet-metal bodies, this has to be a geometric/metadata check the detector runs itself, before (or as part of) the dominant-plane-pair test. A few viable, complementary signals, roughly in order of reliability:

- **`body.type` / body metadata.** Worth confirming empirically what `bodydetails` actually reports for a sheet-metal part's `type` and `properties` fields — Onshape's sheet-metal parts are still `SOLID` bodies at the B-rep level, so `type` alone likely won't distinguish them, but `properties` may carry a sheet-metal-specific marker (e.g. a gauge/thickness property, or a material entry drawn from the sheet-metal material table) worth checking against real fixture data before ruling it out.
- **A second Onshape API call, only for near-miss candidates.** If a body passes the geometric plate test, a cheap follow-up call (e.g. `GET .../parts` or the Part Studio's feature list, similar to what the spacer detector's retired FeatureScript approach used to fetch) can check whether the owning Part Studio has a sheet-metal-flagged feature (a "Convert to sheet metal" or similar feature type) in its history. This costs an extra request per *candidate*, not per row, so it's only paid when the geometry already looks plate-like — acceptable overhead given how request-conscious the existing detectors already are (grouping/caching by Part Studio).
- **Uniform-thickness-with-bend heuristic as a fallback.** If neither metadata signal is available/reliable, a weaker geometric tell: true machined plate stock is a single planar slab, while a folded sheet-metal part — even one that happens to read as "two dominant parallel planes" in one region — will very often have additional face structure (bend reliefs, non-90°-corner transitions, or a third/fourth plane pair at a different in-plane location rather than a different thickness offset) that a strict single-region dominant-plane-pair test would already tend to reject. This isn't a positive sheet-metal identification so much as "the existing geometric strictness incidentally screens most of them out" — not something to rely on alone, but worth noting as a partial backstop while the metadata-based check above is being validated against real fixtures.

Recommended approach for Phase 1: capture `bodydetails` fixtures for at least one confirmed sheet-metal part alongside the plain-plate and counterbore fixtures already planned, and use those to determine which of the above signals actually shows up in the response before committing to one. This is a "look at real data first" question, not a design decision that can be settled purely on paper.
