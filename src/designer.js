...
async function renderAssemblyDetail() {
        ...
        ${currentChildren.map(c => {
          ...
          return `<div class="asm-child-row" data-open-child="${c.childAssemblyId}">
            ...
            <div class="asm-child-text">
              ...
              ${childAsm ? `<div class="asm-child-meta">${statusLabel(childAsm.status)}</div>` : ''}
            </div>
            <div class="asm-child-qty">× ${c.quantity}</div>
            <i class="ti ti-chevron-right" style="color:var(--color-text-tertiary);font-size:13px"></i>
          </div>`
        }).join('')}
      </div>`
    : ''
    ...
    <div class="asm-detail">
      ...
      ${currentParts.length
        ...
        : `<div class="empty" style="padding:40px 0">
            ...
            <div class="empty-title">No direct parts yet</div>
  ...
  area.querySelectorAll('[data-open-child]').forEach(el =>
    el.addEventListener('click', () => selectAssembly(el.dataset.openChild))
  )
