...
// ── Content ───────────────────────────────────────────────────
export function renderDesignerContent() {
...
async function renderAssemblyDetail() {
  ...
  area.innerHTML = `<div class="empty"><i class="ti ti-loader-2 spin"></i><div class="empty-title">Loading parts…</div></div>`

  try {
    currentParts = await fetchAssemblyParts(currentAssemblyId)
  } catch (e) {
    area.innerHTML = `<div class="empty"><i class="ti ti-alert-circle"></i><div class="empty-title">Error loading parts</div></div>`
    return
  }
  ...
  area.innerHTML = `
    <div class="asm-detail">
      <div class="asm-detail-toolbar">
        <button class="btn btn-sm" id="btn-back-asm"><i class="ti ti-arrow-left" aria-hidden="true"></i> All assemblies</button>
        ${linkedBadge}
        <div style="flex:1"></div>
        ${onshapeBtn}
        <button class="btn btn-sm" id="btn-edit-asm"><i class="ti ti-edit" aria-hidden="true"></i> Edit</button>
      ...
      ${currentParts.length
        ...
        : `<div class="empty" style="padding:40px 0">
            ...
            <div class="empty-title">No parts yet</div>
            <div class="empty-sub">Add parts manually, import a CSV, or pull from Onshape.</div>
          </div>`}
    </div>`

  // Bind toolbar buttons
  document.getElementById('btn-back-asm').addEventListener('click', () => selectAssembly(null))
...
function renderOnshapeModal() {
  ...
  const canConfirm = onshapeStep === 'preview' && !onshapeLoading &&
    (onshapeMode === 'link' || onshapePreviewParts.length > 0)
...
function renderOnshapePreviewStep() {
  ...
  const partsCount = onshapePreviewParts.length

  const tableHTML = partsCount
    ? `<div class="parts-table-wrap" style="max-height:260px;overflow-y:auto">
        <table class="parts-table">
          <thead><tr><th>Part name</th><th>Part #</th><th style="text-align:center">Qty</th></tr></thead>
          <tbody>${onshapePreviewParts.map(p => `<tr>
            <td>${p.partName}</td>
            <td><span class="part-number">${p.partNumber || '—'}</span></td>
    ...
    : `<div class="onshape-state" style="padding:20px 0">
        ...
        <div class="onshape-state-sub">No parts were found. You can still create the linked assembly and add parts later.</div>
      </div>`

  if (onshapeMode === 'link') {
    // 'link' mode — create a new Partshelf assembly linked to this Onshape assembly.
    // Show an editable name field above the BOM preview.
    body.innerHTML = `
      ${warningHTML}
      ...
      <div style="font-size:12px;color:var(--color-text-tertiary);margin-bottom:8px">
        <strong>${partsCount}</strong> part${partsCount === 1 ? '' : 's'} will be imported from <strong>${onshapeSelectedAsm?.name}</strong>.
        Parts imported via Onshape link are <em>frozen</em> — they can be cancelled but not deleted.
      </div>
      ${tableHTML}`

    // Keep onshapeCreateName in sync as the user types
    ...
    // 'import' mode — add parts into the currently open assembly.
    body.innerHTML = `
      ${warningHTML}
      ${partsCount ? `<p style="font-size:12px;color:var(--color-text-tertiary);margin-bottom:10px">
        <strong>${onshapeSelectedAsm?.name}</strong> — ${partsCount} part${partsCount === 1 ? '' : 's'} will be added to the current assembly's part list.
      </p>` : ''}
      ${tableHTML}`
  }
}
...
// 'link' mode — create a brand new assembly linked to the chosen Onshape assembly
async function confirmLinkAssembly() {
  ...
  const assemblyPayload = {
    id:                 genId(),
    name,
    description:        `Linked from Onshape: ${asm.name}`,
    onshapeUrl,
  ...
  const savedAssembly = await upsertAssembly(assemblyPayload)
  assemblies.unshift(savedAssembly)

  // Insert BOM parts — marked source:'onshape' so the frozen-BOM feature
  // (coming next) knows these are linked rows
  if (onshapePreviewParts.length) {
    ...
    const savedParts = await bulkInsertAssemblyParts(parts)
    // Navigate into the new assembly — selectAssembly will fetch parts fresh
  }

  closeOnshapeModal()
  selectAssembly(savedAssembly.id)
  toastFn(`Assembly "${name}" created and linked to Onshape`)
}

// 'import' mode — add parts to the currently open assembly (unchanged behaviour)
async function confirmImportParts() {
  ...
  // Attach the Onshape link if this assembly wasn't linked yet
  const assembly = assemblyById(currentAssemblyId)
